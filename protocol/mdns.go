...
func listenMDNS(deviceName string, ctx context.Context) {
...
func sendMdns(addr *net.IPNet, localHaddr net.HardwareAddr, ip utils.IP, mhaddr net.HardwareAddr) {
	srcIp := addr.IP
...
func parseMdns(data []byte) string {
	...
	for s := i - 1; s > 1; s-- {
		...
		if bto16([]byte{data[s-2], data[s-1]}) == uint16(num+7) {
			return utils.ReverseString(buf.String())
		}
