...
func listenNBNS(deviceName string, addr *net.IPNet, ctx context.Context) {
	handle, err := pcap.OpenLive(deviceName, 65536, true, pcap.BlockForever)
	if err != nil {
		log.Fatal(err)
	}
	defer handle.Close()
	handle.SetBPFFilter("udp and port 137 and dst host " + addr.IP.String())
...
func sendNbns(addr *net.IPNet, localHaddr net.HardwareAddr, ip utils.IP, mhaddr net.HardwareAddr) {
	srcIp := addr.IP
	...
	handle, err := pcap.OpenLive(curDeviceName, 65536, true, pcap.BlockForever)
	if err != nil {
		log.Fatal(err)
	}
	defer handle.Close()
